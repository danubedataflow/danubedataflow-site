...
export class Work0033 extends Work {
     ...
     *
     *    lsystem.setProduction('A', () => (MathUtils.random() < borderChangeChance) ? 'B' : 'A')
     *    lsystem.setProduction('B', () => (MathUtils.random() < middleChangeChance) ? 'A' : 'C')
     *    lsystem.setProduction('C', () => (MathUtils.random() < middleChangeChance) ? 'B' : 'D')
     *    lsystem.setProduction('D', () => (MathUtils.random() < middleChangeChance) ? 'C' : 'E')
     *    lsystem.setProduction('E', () => (MathUtils.random() < borderChangeChance) ? 'D' : 'E')
     */
    makeLsystem() {
        ...
        for (let i = 0; i < numSymbols; i++) {
            ...
            if (i == 0) {
                lsystem.setProduction(symbol, () => (MathUtils.random() < borderChangeChance) ? nextSymbol : symbol)
            } else if (i == numSymbols - 1) {
                lsystem.setProduction(symbol, () => (MathUtils.random() < borderChangeChance) ? prevSymbol : symbol)
            } else {
                lsystem.setProduction(symbol, () => (MathUtils.random() < middleChangeChance) ? prevSymbol : nextSymbol)
            }
            charCode++;
        }
    ...
    makeAxiom(symbolsForAxiom, length) {
        ...
        for (var i = 0; i < length; i++) {
            result += symbolsForAxiom.charAt(Math.floor(MathUtils.random() * symbolsForAxiom.length));
        }
        return result;
    }
